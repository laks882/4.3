...
const USAGE_UNIT_NAME = "ENRICHMENT_CREDITS";
const RECORDS_PER_UNIT = 100; // Charge per 100 records
...
const DISCORD_WEBHOOK_URL =
  "https://discord.com/api/webhooks/1393445359191326720/yU9TVN53T9NthPKUKz_K0cgfnExIY13cVzBuVL8OffILCr2Zi-4kn7gyiqKWkgbLJ4I-";
  ...
  try {
    ...
    const title =
      type === "Failed"
        ...
        : "⏹️ Enrichment Request Cancelled";
    ...
    const embed = {
      ...
      fields: [
        ...
        },
        {
          ...
          value:
            responseData.requested_leads_count ||
            requestData.noOfLeads?.toString() ||
            "Unknown",
          inline: true,
        },
        {
        ...
        },
        {
          name: "Apollo Link",
          value:
            responseData.apollo_link ||
            requestData.apolloLink ||
            "Not provided",
          inline: false,
        },
        {
          ...
          value:
            responseData.failure_time ||
            responseData.cancelled_time ||
    ...
    const payload = {
      content:
        type === "Failed"
          ? "⚠️ **Enrichment Request Failed**"
          : "🛑 **Enrichment Request Cancelled**",
      embeds: [embed],
    };
...
async function checkEnrichmentStatus(logId, originalRequest) {
  ...
  while (retries < maxRetries) {
    try {
      ...
      const data = Array.isArray(statusRes.data)
        ? statusRes.data[0]
        : statusRes.data;
      ...
      if (!data) {
        console.log(
          `⚠️ Warning: No data received in status response (Attempt ${
            retries + 1
          }/${maxRetries})`
        );
        await sleep(10000);
      ...
      if (status && status.toLowerCase() === "completed") {
        result = data;
        console.log(
          "✅ Enrichment completed successfully! Stopping polling immediately."
        );
      ...
      if (status && status.toLowerCase() === "cancelled") {
        ...
        throw new Error(
          `Enrichment cancelled: ${
            data.cancellation_reason || "Unknown reason"
          }`
        );
      }
      ...
      if (
        ...
        console.log(
          `⚠️ Unknown status received: ${status} - Continuing to poll...`
        );
      }
    } catch (error) {
      if (
        error.message.includes("Enrichment failed") ||
        error.message.includes("Enrichment cancelled")
      ) {
        throw error; // Re-throw handled errors
      }

      console.error(
        `🚨 Error during status check (Attempt ${retries + 1}/${maxRetries}):`,
        error.message
      );
  ...
  if (!result) {
    const timeoutError = new Error(
      `Timeout: No completion status received after ${maxRetries} attempts (${Math.round(
        (maxRetries * 10) / 60
      )} minutes)`
    );
...
await Actor.main(async () => {
  ...
  if (!input) {
    throw new Error(
      "No input provided. Please provide apolloLink, noOfLeads, and fileName."
    );
  }
  ...
  if (!logId) {
    throw new Error(
      "Failed to get LogID from enrichment request. Response: " +
        JSON.stringify(startRes.data)
    );
  }

  console.log(
    `✅ Enrichment request submitted successfully. Record ID: ${logId}`
  );
  ...
  // Track usage based on credits_involved
  if (result.credits_involved > 0) {
    const chargeResult = await Actor.charge({
      eventName: USAGE_UNIT_NAME,
      count: result.credits_involved,
    });
    console.log(`✅ Usage tracked: ${result.credits_involved} credits`);
    console.log(`💰 Charge result:`, chargeResult);
  } else {
    console.log("⚠️ No credits consumed - no usage charged");
  }
  ...
  console.log(
    "📋 You can access the enriched data at the spreadsheet URL above."
  );
  return result;
});
